...
fromYamlFile(yamlFileMap0).then((world) => {
  const game = new Game({ "world": new WorldScene(world), "gameover": new GameOverScene(), "chooseRoles": new ChooseRolesScene() } as Record<SceneName, Scene>);
  game.initialize();
  game.start(loader).then(() => {
    // after player clicks start game, for example
    //game.goToScene('world' as SceneName);
    game.goToScene('chooseRoles' as SceneName);
  });
})
