...
interface CacheItem {
    lastUpdate: Date;
    entitiesMemo: WeakMap<Actor, {lastSeen: Date, pos: Vector, estimatedDistance: number}>;
}
...
export function buildPerception(dummy: Dummy, guard: Guard): AiPerception {
    ...
    const now = new Date();
    const entities = guard.getClosestEntities(dummy.pos, 1500).filter((e) => e !== dummy && e instanceof Dummy).map((entity) => {
    ...
    }).filter(lastInfo.entitiesMemo.has).toSorted((e1, e2) => {
        const d1 = lastInfo.entitiesMemo.get(e1)!;
        const d2 = lastInfo.entitiesMemo.get(e2)!;
        let diff = Math.round((d2.lastSeen.getTime() - d1.lastSeen.getTime()) / 1000);
        if(diff === 0) {
    ...
    return {
        ...
        enemyClosestKnownDistance: entities.length > 0 ? closestInfo!.estimatedDistance : Infinity,
        enemyClosestLastSeen: entities.length > 0 ? (now.getTime() - closestInfo!.lastSeen.getTime()) : Infinity,
