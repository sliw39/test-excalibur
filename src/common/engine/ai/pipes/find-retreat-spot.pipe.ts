...
export class FindRetreatSpotPipe extends PointFinderPipe {
  constructor(behavior: Behavior, private _distance: number = 500) {
    super("move_away", behavior);
  }

  probability(ai: AiPerception): number {
    const aggFoes = ai.foes.filter(
      (foe) =>
        foe.currentWeapon.currentState instanceof AimingState ||
        foe.currentWeapon.currentState instanceof FiringState
    ).length;
    if (aggFoes === 0) {
      return 1;
    } else if (aggFoes === 1) {
  ...
  execute(ai: AiPerception): Promise<void> {
    ...
    const awayDirection = enemiesAvgPosition
      .sub(ai.player.pos)
      .normalize()
      .scale(-1);
